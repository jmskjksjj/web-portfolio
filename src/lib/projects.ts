...
export const projects: Project[] = [
  ...
  // ── App ──
  {
    ...
    metrics: [
      ...
      { label: "MCP Tools", value: "49", labelKo: "MCP 도구" },
  ...
  },
  {
    ...
    category: "mcp",
    type: "MCP Server · Site Analysis + KOSIS Statistics",
    typeKo: "MCP 서버 · 대지분석 + KOSIS 통계",
    description:
      "Phase 2 site-analysis engine (v2.5). Combines NGII topographic ZIP-based geospatial analysis (9 analyzer modules) with KOSIS national statistics (196-item building-use matrix) and external APIs (VWorld, Open-Meteo, Claude). Outputs an enriched site_constraints.json with AI-generated design recommendations.",
    descriptionKo:
      "Phase 2 대지분석 엔진 (v2.5). NGII 수치지형도 ZIP 기반 9개 분석 모듈에 KOSIS 국가통계(건물 용도별 196개 데이터 항목)와 외부 API(VWorld·Open-Meteo·Claude)를 결합. AI 설계 권고가 포함된 enriched site_constraints.json을 출력합니다.",
    detail:
      "The analytical brain of the \"Archflow\" pipeline's Phase 2. Operates as the SiteAgent's second call after arch-site-mcp builds the 3D model: this server runs 9 analyzer modules (terrain, climate, context, accessibility, view, environment, regulatory, urban-context, site-strategy) plus KOSIS demographic enrichment, then merges everything into one enriched site_constraints.json. Climate data comes from Open-Meteo Archive API (free, no key), regulatory data from VWorld land-use API, jurisdiction-specific demographics from KOSIS API (filtered through a 196-item matrix tailored per building use), and design recommendations are AI-synthesized via Claude. The analyzers themselves are pure functions — geometric operations on Pydantic v2 schemas (19 output models) covering 113 pytest tests. Built so each analyzer runs independently for re-analysis, but the master analyze_site call orchestrates all 9 modules + KOSIS in one shot.",
    detailKo:
      "\"Archflow\" 파이프라인 Phase 2의 분석 두뇌. SiteAgent가 arch-site-mcp로 3D 모델을 만든 뒤 두 번째로 호출하는 서버 — 9개 분석 모듈(지형·기후·주변환경·접근성·조망·환경·법규·도시맥락·배치전략)과 KOSIS 인구통계 보강을 실행한 후, enriched site_constraints.json 하나로 병합합니다. 기후 데이터는 Open-Meteo Archive API(무료·무키), 법규 데이터는 VWorld 토지이용계획 API, 지역별 인구통계는 KOSIS API(건물 용도별 196개 항목 매트릭스로 필터링), 설계 권고는 Claude API로 AI 합성합니다. 9개 분석기는 순수 함수로 구현되어 Pydantic v2 스키마(19개 출력 모델) + 113개 pytest 테스트로 검증됩니다. 각 분석기는 독립 재분석이 가능하지만, 마스터 analyze_site 호출이 9개 모듈과 KOSIS를 한 번에 오케스트레이션합니다.",
    techStack: ["Python 3.11", "FastMCP", "Pydantic v2", "GeoPandas", "Shapely", "Open-Meteo API", "KOSIS API", "VWorld API", "Claude API"],
    metrics: [
      { label: "Tools", value: "13", labelKo: "도구" },
      { label: "Analyzers", value: "9", labelKo: "분석 모듈" },
      { label: "KOSIS Items", value: "196", labelKo: "KOSIS 데이터 항목" },
      { label: "Pytest Tests", value: "113", labelKo: "테스트" },
    ],
    status: "production",
    tools: [
      {
        name: "analyze_site",
        description: "Master orchestrator — runs all 9 analyzers + KOSIS demographic enrichment in one call, then merges results with arch-site-mcp's site_area/boundaries into an enriched site_constraints.json. The SiteAgent's primary entry point.",
        descriptionKo: "마스터 오케스트레이터 — 9개 분석기 + KOSIS 인구통계 보강을 한 번에 실행하고, arch-site-mcp의 site_area/boundaries와 병합하여 enriched site_constraints.json을 생성합니다. SiteAgent의 주요 진입점.",
        input: "ZIP file paths, optional compliance, address, model_path, site_area_input, boundaries_input",
        inputKo: "ZIP 파일 경로, 법규(선택), 주소, model_path, site_area_input, boundaries_input",
        output: "Enriched site_constraints.json with 9-analyzer + KOSIS results + AI design recommendations",
        outputKo: "9개 분석기 + KOSIS 결과 + AI 설계 권고가 포함된 enriched site_constraints.json",
      },
      {
        name: "extract_gis_data",
        description: "Developer-only utility. Extracts GIS features from NGII topographic ZIP files (EUC-KR encoding) into a GeoJSON summary. Supports filtering by layer name (B001 buildings, A001 roads, F001 contours, etc.).",
        descriptionKo: "개발자 전용 유틸리티. NGII 수치지형도 ZIP(EUC-KR)에서 GIS 피처를 GeoJSON 요약으로 추출합니다. 레이어명 필터링 지원 (B001 건물, A001 도로, F001 등고선 등).",
        input: "NGII ZIP file paths, optional layer name filter",
        inputKo: "NGII ZIP 파일 경로, 레이어명 필터(선택)",
        output: "GeoJSON summary of extracted features per layer",
        outputKo: "레이어별 추출 피처의 GeoJSON 요약",
      },
      {
        name: "analyze_terrain_tool",
        description: "Computes terrain characteristics — slope gradient, elevation distribution, drainage flow direction — using Delaunay triangulation on F001 contour data within the site boundary.",
        descriptionKo: "지형 분석 — 경사도, 표고, 배수방향을 계산합니다. 등고선 F001 데이터에 Delaunay 삼각분할을 적용해 산출.",
        input: "NGII ZIP file paths",
        inputKo: "NGII ZIP 파일 경로",
        output: "TerrainAnalysis (slope class, elevation stats, drainage direction)",
        outputKo: "TerrainAnalysis (경사 분류, 표고 통계, 배수 방향)",
      },
      {
        name: "analyze_climate_tool",
        description: "Computes climate analysis — wind rose, daylight, temperature — using Open-Meteo Archive API (free, no key required). Includes solar path calculations for winter solstice, equinoxes, and summer solstice.",
        descriptionKo: "기후 분석 — 풍배도, 일조, 기온을 계산합니다. Open-Meteo Archive API(무료·무키)와 동지/춘추분/하지 태양경로 계산 포함.",
        input: "NGII ZIP file paths OR direct lat/lon coordinates",
        inputKo: "NGII ZIP 파일 경로 또는 위도·경도 직접 입력",
        output: "ClimateAnalysis (wind rose, daylight hours, temperature, sun path)",
        outputKo: "ClimateAnalysis (풍배도, 일조시간, 기온, 태양경로)",
      },
      {
        name: "analyze_context_tool",
        description: "Analyzes surrounding context within a radius — neighbor building heights/uses/density and shadow influence on the site, computed from B001 building layer.",
        descriptionKo: "주변환경 분석 — 반경 내 주변 건물 높이/용도/밀도 및 대지에 미치는 그림자 영향을 계산합니다. 건물 B001 레이어 기반.",
        input: "NGII ZIP file paths, optional radius (m)",
        inputKo: "NGII ZIP 파일 경로, 반경(선택, m)",
        output: "ContextAnalysis (neighbor heights, density, shadow zones)",
        outputKo: "ContextAnalysis (주변 건물 높이, 밀도, 그림자 구역)",
      },
      {
        name: "analyze_accessibility_tool",
        description: "Computes accessibility — road frontage, access direction, front-road classification (pedestrian/vehicle) — from A001 road layer. Critical for Korean Building Act setback and parking-access rules.",
        descriptionKo: "접근성 분석 — 도로 접면, 접근 방향, 전면도로(보행/차량) 분류를 계산합니다. 도로 A001 기반. 건축법 이격·주차 출입 규정 준수에 필수.",
        input: "NGII ZIP file paths",
        inputKo: "NGII ZIP 파일 경로",
        output: "AccessibilityAnalysis (road frontage, access direction, road type)",
        outputKo: "AccessibilityAnalysis (도로 접면, 접근 방향, 도로 유형)",
      },
      {
        name: "analyze_views_tool",
        description: "Computes view analysis — 8-direction openness scores and view corridors — from B001 building layer within the analysis radius.",
        descriptionKo: "조망 분석 — 8방위 개방감 점수, 조망 축을 계산합니다. 분석 반경 내 건물 B001 기반.",
        input: "NGII ZIP file paths, optional radius (m)",
        inputKo: "NGII ZIP 파일 경로, 반경(선택, m)",
        output: "ViewAnalysis (8-direction openness scores, view corridors)",
        outputKo: "ViewAnalysis (8방위 개방감 점수, 조망 축)",
      },
      {
        name: "analyze_environment_tool",
        description: "Computes environment analysis — green coverage ratio, water proximity, noise estimation — from D003 (vegetation), E001 (water), and A001 (road) layers.",
        descriptionKo: "환경 분석 — 녹지율, 수계 근접도, 소음 추정을 계산합니다. D003(식생)·E001(수계)·A001(도로) 레이어 기반.",
        input: "NGII ZIP file paths, optional radius (m)",
        inputKo: "NGII ZIP 파일 경로, 반경(선택, m)",
        output: "EnvironmentAnalysis (green ratio, water proximity, noise estimate)",
        outputKo: "EnvironmentAnalysis (녹지율, 수계 근접도, 소음 추정)",
      },
      {
        name: "analyze_regulatory_tool",
        description: "Regulatory overlay analysis — zoning classification, building line setback, buildable area — using VWorld land-use API + H001 layer + arch-law-mcp compliance data.",
        descriptionKo: "법규 오버레이 분석 — 용도지역, 건축선 후퇴, 가용 건축면적을 계산합니다. VWorld 토지이용계획 API + H001 레이어 + arch-law-mcp 법규 데이터 결합.",
        input: "NGII ZIP file paths, optional compliance data from arch-law-mcp",
        inputKo: "NGII ZIP 파일 경로, arch-law-mcp 법규 데이터(선택)",
        output: "RegulatoryAnalysis (zoning, setback, buildable area)",
        outputKo: "RegulatoryAnalysis (용도지역, 건축선 후퇴, 가용 건축면적)",
      },
      {
        name: "analyze_urban_context_tool",
        description: "Urban-context analysis — green/pedestrian/vehicle axes, key facilities, block position. Goes beyond immediate context to understand the site's role in the broader urban fabric.",
        descriptionKo: "도시맥락 분석 — 녹지축/보행축/차량축, 주요시설, 블록위치를 분석합니다. 즉각적 주변환경을 넘어 더 넓은 도시 구조 내 대지의 역할 파악.",
        input: "NGII ZIP file paths, optional radius (m)",
        inputKo: "NGII ZIP 파일 경로, 반경(선택, m)",
        output: "UrbanContextAnalysis (urban axes, facilities, block position)",
        outputKo: "UrbanContextAnalysis (도시축, 시설, 블록위치)",
      },
      {
        name: "analyze_site_strategy_tool",
        description: "Site placement strategy — synthesizes all 7 sub-analyses + urban context into a strategic placement recommendation. Uses Claude API for AI-driven strategy when configured, otherwise falls back to rule-based logic.",
        descriptionKo: "배치전략 분석 — 7개 분석 + 도시맥락을 종합하여 건축 배치전략을 생성합니다. Claude API가 설정되어 있으면 AI 기반 전략, 없으면 규칙 기반 폴백.",
        input: "NGII ZIP file paths, optional compliance + address",
        inputKo: "NGII ZIP 파일 경로, 법규 + 주소(선택)",
        output: "SiteStrategy (placement zones, mass distribution, orientation)",
        outputKo: "SiteStrategy (배치 영역, 매스 분배, 향)",
      },
      {
        name: "get_kosis_data_matrix",
        description: "Returns a built-in matrix of KOSIS data items available for a given building use type — filtered by priority (1=must-have, 2=recommended, 3=optional). 196 items across all use types. No API calls — instant response.",
        descriptionKo: "건물 용도별 조회 가능한 KOSIS 데이터 항목 목록을 반환합니다. 우선순위 필터(1=필수, 2=권장, 3=선택), 모든 용도 합쳐 196개 항목. API 호출 없이 즉시 응답.",
      ...
      },
      {
        ...
        description: "Fetches actual KOSIS statistical values via API. Auto-resolves address to municipality code (5-digit) for area-specific queries. Combines selected items + common items into a unified dataset for design-decision support.",
        descriptionKo: "KOSIS API로 실제 통계 값을 조회합니다. 주소 → 시군구코드(5자리) 자동 변환으로 지역별 쿼리. 선택 항목 + 공통 항목을 통합 데이터셋으로 결합해 설계 의사결정 지원.",
